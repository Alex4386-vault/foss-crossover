...
void Preprocessor::EnterSourceFileWithLexer(Lexer *TheLexer,
  ...
  SrcMgr::CharacteristicKind PrevFileType = SrcMgr::C_LocalUser;
  if (Callbacks && CurLexer && !CurLexer->Is_PragmaLexer)
    PrevFileType = SourceMgr.getFileCharacteristic(CurLexer->getFileLoc());
  ...
  if (Callbacks && !CurLexer->Is_PragmaLexer) {
    ...
    Callbacks->FileChanged(CurLexer->getFileLoc(),
                           PPCallbacks::EnterFile, FileType, PrevFileType);
  }
}
...
void Preprocessor::EnterSourceFileWithPTH(PTHLexer *PL,
  ...
  SrcMgr::CharacteristicKind PrevFileType = SrcMgr::C_LocalUser;
  if (Callbacks && CurPPLexer)
    PrevFileType =
        SourceMgr.getFileCharacteristic(SourceMgr.getLocForStartOfFile(
            CurPPLexer->getFileID()));
    ...
    if (Callbacks && !isEndOfMacro && CurPPLexer) {
      ...
      SrcMgr::CharacteristicKind PrevFileType =
        SourceMgr.getFileCharacteristic(
            SourceMgr.getLocForEndOfFile(ExitedFID));
      Callbacks->FileChanged(CurPPLexer->getSourceLocation(),
                             PPCallbacks::ExitFile, FileType, PrevFileType,
                             ExitedFID);
    }
