...
void Preprocessor::HandleLineDirective() {
  ...
  if (Callbacks)
    Callbacks->FileChanged(CurPPLexer->getSourceLocation(),
                           PPCallbacks::RenameFile, FileKind, FileKind);
}
...
static bool ReadLineMarkerFlags(bool &IsFileEntry, bool &IsFileExit,
  ...
  // We must have 3 or 5 if there are still flags.
  if (FlagVal != 3 && FlagVal != 5) {
    PP.Diag(FlagTok, diag::err_pp_linemarker_invalid_flag);
    PP.DiscardUntilEndOfDirective();
    return true;
  }

  FileKind = FlagVal == 3 ? SrcMgr::C_System : SrcMgr::C_LocalUser;
  ...
  // For system headers, we must have 4 if there is yet another flag. There
  // are no more valid flags for local headers.
  if (FlagVal != 4 || FileKind == SrcMgr::C_LocalUser) {
    PP.Diag(FlagTok, diag::err_pp_linemarker_invalid_flag);
    PP.DiscardUntilEndOfDirective();
    return true;
  }
...
void Preprocessor::HandleDigitDirective(Token &DigitTok) {
  ...
  SrcMgr::CharacteristicKind PrevFileKind =
      SourceMgr.getFileCharacteristic(DigitTok.getLocation());
...
void Preprocessor::HandleIncludeDirective(SourceLocation HashLoc,
  ...
  if (CheckIncludePathPortability) {
    ...
    if (trySimplifyPath(Components, RealPathName)) {
      ...
      auto DiagId = (!SrcMgr::isSystem(FileCharacter) || warnByDefaultOnWrongCase(Name)) ?
