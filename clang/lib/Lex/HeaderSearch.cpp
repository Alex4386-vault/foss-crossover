      ...
      if (const DirectoryEntry *FrameworkDir
        ...
        bool IsSystem
          = SearchDirs[Idx].getDirCharacteristic() != SrcMgr::C_User &&
            SearchDirs[Idx].getDirCharacteristic() != SrcMgr::C_LocalUser;
...
const FileEntry *DirectoryLookup::DoFrameworkLookup(
  ...
  if (!CacheEntry.Directory) {
    ...
    if (getDirCharacteristic() == SrcMgr::C_User ||
        getDirCharacteristic() == SrcMgr::C_LocalUser) {
...
const FileEntry *HeaderSearch::LookupFile(
    ...
    for (const auto &IncluderAndDir : Includers) {
      ...
      bool IncluderIsSystemHeader =
          Includer ? (getFileInfo(Includer).DirInfo != SrcMgr::C_User &&
                      getFileInfo(Includer).DirInfo != SrcMgr::C_LocalUser) :
          BuildSystemModule;
  ...
  for (; i != SearchDirs.size(); ++i) {
    ...
    // characteristic.
    if ((HFI.DirInfo == SrcMgr::C_User || HFI.DirInfo == SrcMgr::C_LocalUser) &&
        InUserSpecifiedSystemFramework)
