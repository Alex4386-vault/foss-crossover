...
static Value *emitPointerArithmetic(CodeGenFunction &CGF,
  ...
  if (elementType->isVoidType() || elementType->isFunctionType()) {
    // Use a void pointer type in the appropriate address space.
    llvm::Type *pointerTy = pointer->getType();
    // Can't use CGF.VoidTy here because getPointerTo() asserts
    llvm::Type *voidPtrPointeeTy = CGF.VoidPtrTy->getElementType();
    llvm::Type *voidPtrTy = voidPtrPointeeTy->getPointerTo(pointerTy->getPointerAddressSpace());
    Value *result = CGF.Builder.CreateBitCast(pointer, voidPtrTy);
    result = CGF.Builder.CreateGEP(result, index, "add.ptr");
    return CGF.Builder.CreateBitCast(result, pointerTy);
  }
