...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
    ...
    ForDefinition_t IsForDefinition, unsigned AddressSpace) {
  const Decl *D = GD.getDecl();
  ...
  if (Entry) {
    ...
    if (!IsForDefinition)
    {
      if (Entry->getType()->getAddressSpace() != 0)
        return llvm::ConstantExpr::getAddrSpaceCast(Entry, Ty->getPointerTo());
  ...
  llvm::Function *F =
      llvm::Function::Create(FTy, llvm::Function::ExternalLinkage, AddressSpace,
  ...
  if (Entry) {
    ...
    unsigned EntryAS = 0;
    if (llvm::PointerType *PTy = dyn_cast<llvm::PointerType>(Entry->getType()))
        EntryAS = PTy->getAddressSpace();
    llvm::Constant *BC = llvm::ConstantExpr::getBitCast(
        F, Entry->getType()->getElementType()->getPointerTo(EntryAS));
...
llvm::Constant *CodeGenModule::GetAddrOfFunction(GlobalDecl GD,
  ...
  if (!Ty) {
    auto CanonTy = Context.getCanonicalType(FDQT);
  ...
  StringRef MangledName = getMangledName(GD);
  unsigned AS = static_cast<unsigned>(-1);
  // Make sure the function is in the correct AS.
  if (LangOpts.Interop6432) {
    Qualifiers Quals = FDQT.getQualifiers();
    if (Quals.hasAddressSpace() || Quals.hasLatentAddressSpace()) {
      LangAS LAS = Quals.hasAddressSpace() ? Quals.getAddressSpace() :
                                             Quals.getLatentAddressSpace();
      AS = Context.getTargetAddressSpace(LAS);
    }
  }
...
void CodeGenModule::AddVTableTypeMetadata(llvm::GlobalVariable *VTable,
  ...
  llvm::Metadata *MD =
      CreateMetadataIdentifierForType(RD->getQualTypeForDecl());
