...
static Address emitBuiltinRetPointerCast(CodeGenFunction &CGF, Address Dest, const CallExpr *E)
{
  llvm::Type *DestTy = Dest.getPointer()->getType();
  llvm::Type *RetTy = CGF.ConvertType(E->getType());

  if (RetTy != DestTy) {
    unsigned AS = RetTy->getPointerAddressSpace();
    if (AS != DestTy->getPointerAddressSpace())
      Dest = CGF.Builder.CreateAddrSpaceCast(Dest, DestTy->getPointerTo(AS));
  }
...
RValue CodeGenFunction::EmitBuiltinExpr(const FunctionDecl *FD,
  ...
  case Builtin::BI__builtin_memcpy: {
    ...
    Dest = emitBuiltinRetPointerCast(*this, Dest, E);
