...
public:
  X86_64ABIInfo(CodeGen::CodeGenTypes &CGT, X86AVXABILevel AVXLevel) :
      ...
      IsWine32(getTarget().getTriple().is32On64()),
      IsStack64(
          getTarget().getTriple().getEnvironment() == llvm::Triple::Wine32SP64){
    if (IsWine32) {
  ...
  bool isWine32SP64Target() const { return IsStack64; }
};
...
public:
  ...
  LangAS getASTAllocaAddressSpace() const override {
    if (getABIInfo().isWine32Target() && !getABIInfo().isWine32SP64Target())
      return LangAS::ptr32;
...
GetINTEGERTypeAtOffset(llvm::Type *IRType, unsigned IROffset,
  ...
  if (IROffset == 0) {
    ...
    if (IRType->isIntegerTy(64))
      return IRType;
    if (llvm::PointerType *PTy = dyn_cast<llvm::PointerType>(IRType)) {
      if (Has64BitPointers && (!IsWine32 || PTy->getAddressSpace() != 32))
        return IRType;
      if (BitsContainNoUserData(SourceTy, SourceOffset*8+32,
                                SourceOffset*8+64, getContext()))
        return IRType;
