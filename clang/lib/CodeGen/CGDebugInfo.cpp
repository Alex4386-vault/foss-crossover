...
llvm::DIType *CGDebugInfo::CreateEnumType(const EnumType *Ty) {
  ...
  uint64_t Size = 0;
  uint32_t Align = 0;
  if (!ED->getQualTypeForDecl()->isIncompleteType()) {
    Size = CGM.getContext().getTypeSize(ED->getQualTypeForDecl());
...
void CGDebugInfo::CollectContainingType(const CXXRecordDecl *RD,
  ...
  if (const CXXRecordDecl *PBase = RL.getPrimaryBase()) {
    ...
    ContainingType = cast<llvm::DICompositeType>(
        getOrCreateType(PBase->getQualTypeForDecl(),
...
void CGDebugInfo::EmitGlobalVariable(const ValueDecl *VD, const APValue &Init) {
  ...
  if (const auto *ECD = dyn_cast<EnumConstantDecl>(VD)) {
    ...
    assert(isa<EnumType>(ED->getQualTypeForDecl()) && "Enum without EnumType?");
    Ty = getOrCreateType(ED->getQualTypeForDecl(), Unit);
  }
