...
void ClangToLLVMArgMapping::construct(const ASTContext &Context,
  ...
  if (is32BitInteropCC(FI, Context.getTargetInfo().getTriple().is32On64())) {
...
CodeGenTypes::GetFunctionType(const CGFunctionInfo &FI) {
  ...
  if (IRFunctionArgs.hasSRetArg()) {
    QualType Ret = FI.getReturnType();
    llvm::Type *Ty = ConvertType(Ret);
    unsigned AddressSpace = Context.getTargetAddressSpace(
        Context.getTargetInfo().getStackAddressSpace(Context.getLangOpts()));
...
void CodeGenModule::ConstructAttributeList(
  ...
  if (!AttrOnCallSite) {
    ...
    if (TargetDecl)
      if (auto *A = TargetDecl->getAttr<Ptr32ThunkPrefixAttr>())
        FuncAttrs.addAttribute("thunk-prefix", A->getPrefix());
  }
...
RValue CodeGenFunction::EmitCall(const CGFunctionInfo &CallInfo,
  ...
  if (IRFunctionArgs.hasThunkDataArg()) {
    if (!ThunkData.isValid()) {
      llvm::Type *Ty = llvm::StructType::get(llvm::ArrayType::get(Int64Ty, 6));
      ThunkData = CreateTempAllocaWithoutCast(
          Ty, CharUnits::fromQuantity(8), "thunk.storage");
      if (getTarget().getTriple().getEnvironment() == llvm::Triple::Wine32SP64){
        // In this case, the thunk data will be in address space 0, but we need
        // it to be in 32. Put a cast in.
        // Note that just using CreateTempAlloca(), which inserts a similar
        // cast, actually won't work here. It only inserts the cast if the AS
        // as represented in the AST is different from the AS in the LLVM
        // DataLayout. For Stack64, this isn't true.
        // Make sure it's inserted at the alloca point, to ensure it dominates
        // all of its uses.
        llvm::IRBuilderBase::InsertPointGuard IPG(Builder);
        Builder.SetInsertPoint(AllocaInsertPt);
        ThunkData = Address(getTargetHooks().performAddrSpaceCast(
            *this, ThunkData.getPointer(), getASTAllocaAddressSpace(),
            LangAS::ptr32, Ty->getPointerTo(32), /*non-null*/ true),
            ThunkData.getAlignment());
      }
    }
