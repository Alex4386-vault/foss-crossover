...
static bool unionHasUniqueObjectRepresentations(const ASTContext &Context,
  ...
  CharUnits UnionSize = Context.getTypeSizeInChars(RD->getQualTypeForDecl());
...
QualType ASTContext::getLatentAddrSpaceQualType(QualType T,
                                                LangAS AddressSpace) const {
  Qualifiers Quals = T.getQualifiers();
  if (Quals.hasLatentAddressSpace() && Quals.getLatentAddressSpace() == AddressSpace)
    return T;

  // If we are composing extended qualifiers together, merge together
  // into one ExtQuals node.
  QualifierCollector LocalQuals;
  const Type *TypeNode = LocalQuals.strip(T);

  // If this type already has an address space specified, it cannot get
  // another one.
  assert(!LocalQuals.hasLatentAddressSpace() &&
         "Type cannot be in multiple addr spaces!");
  LocalQuals.addLatentAddressSpace(AddressSpace);

  return getExtQualType(TypeNode, LocalQuals);
}

QualType ASTContext::removeAddrSpaceQualType(QualType T) const {
  // If the qualifier doesn't have an address space just return it.
  Qualifiers Quals = T.getQualifiers();
  if (!Quals.hasAddressSpace() && !Quals.hasLatentAddressSpace())
    return T;
  ...
  if (desugar.getTypePtr() != split.Ty) {
    QualType NewT = getQualifiedType(desugar, split.Quals);
    NewT = removeAddrSpaceQualType(NewT);
    switch (split.Ty->getTypeClass()) {
    case Type::Paren:
      split.Ty = getParenType(NewT).getTypePtr();
      break;
    case Type::Decayed:
      split.Ty = getDecayedType(NewT).getTypePtr();
      break;
    case Type::Elaborated:
      split.Ty = getElaboratedType(
          split.Ty->castAs<ElaboratedType>()->getKeyword(),
          split.Ty->castAs<ElaboratedType>()->getQualifier(),
          NewT).getTypePtr();
      break;
    default:
      split.Ty = NewT.getTypePtr();
      break;
    }
  }
  ...
  // or required.
  if (split.Quals.hasNonFastQualifiers())
    return getExtQualType(split.Ty, split.Quals);
  else
    return QualType(split.Ty, split.Quals.getFastQualifiers());
}
...
QualType ASTContext::getFunctionType(QualType ResultTy, ArrayRef<QualType> Args,
                                     const FunctionProtoType::ExtProtoInfo &EPI,
                                     Qualifiers qs) const {
  return getQualifiedType(getFunctionTypeInternal(ResultTy, Args, EPI, false), qs);
}

QualType ASTContext::adjustFunctionType(QualType QT,
                                        FunctionType::ExtInfo Info) {
  const FunctionType *T = QT->castAs<FunctionType>();
  ...
  if (const auto *FNPT = dyn_cast<FunctionNoProtoType>(T)) {
    ...
    EPI.ExtInfo = Info;
    Result = getFunctionType(FPT->getReturnType(), FPT->getParamTypes(), EPI, QT.getQualifiers());
  }
...
const FunctionType *ASTContext::adjustFunctionType(const FunctionType *T,
                                                   FunctionType::ExtInfo Info) {
  return adjustFunctionType(QualType(T, 0), Info)->castAs<FunctionType>();
}
...
void ASTContext::adjustDeducedFunctionResultType(FunctionDecl *FD,
  ...
  while (true) {
    ...
    FunctionProtoType::ExtProtoInfo EPI = FPT->getExtProtoInfo();
    FD->setType(getFunctionType(ResultType, FPT->getParamTypes(), EPI, T.getQualifiers()));
...
QualType ASTContext::getFunctionTypeWithExceptionSpec(
  ...
  if (const auto *PT = dyn_cast<ParenType>(Orig))
    return getQualifiedType(getParenType(
        getFunctionTypeWithExceptionSpec(PT->getInnerType(), ESI)),
        Orig.getQualifiers());

  // Might have a calling-convention attribute.
  if (const auto *AT = dyn_cast<AttributedType>(Orig))
    return getQualifiedType(getAttributedType(
        AT->getAttrKind(),
...
static QualType addressSpaceAdjustedType(const ASTContext &ctx, QualType T)
{
  if (T.getQualifiers().hasAddressSpace())
    return T;

  const FunctionType *FT = T->getAs<FunctionType>();
  if (FT && is32BitInteropCC(FT->getCallConv()))
    T = ctx.getAddrSpaceQualType(T, LangAS::ptr32);
  else if (T.getQualifiers().hasLatentAddressSpace()) {
    // Don't apply latent address space if both pointer and storage address
    // spaces are the default.
    if (ctx.DefaultAddrSpace == LangAS::Default && ctx.StorageAddrSpace == LangAS::Default)
      return addressSpaceAdjustedType(ctx, ctx.removeAddrSpaceQualType(T));

    LangAS AS = T.getQualifiers().getLatentAddressSpace();
    if (AS == LangAS::Default)
      T = ctx.removeAddrSpaceQualType(T);
    else
      T = ctx.getAddrSpaceQualType(T, AS);
  }
  else if (!isTypeDefinedInSystemHeader(ctx, T) &&
           ctx.DefaultAddrSpace != LangAS::Default)
    T = ctx.getAddrSpaceQualType(T, ctx.DefaultAddrSpace);
  else if (isTypeDefinedInSystemHeader(ctx, T) &&
           ctx.getTargetInfo().getTargetOpts().SystemAddrSpace != LangAS::Default)
    T = ctx.getAddrSpaceQualType(T,
                                 ctx.getTargetInfo().getTargetOpts().SystemAddrSpace);
...
QualType ASTContext::getPointerType(QualType T, bool HonorASPragma) const {
  if (HonorASPragma)
    T = addressSpaceAdjustedType(*this, T);
  ...
  QualType Canonical;
  if (!T.isCanonical()) {
    Canonical = getPointerType(getCanonicalType(T), /*HonorASPragma=*/false);
...
QualType ASTContext::getDecayedType(QualType T, bool HonorASPragma) const {
  ...
  if (T->isArrayType())
    Decayed = getArrayDecayedType(T, HonorASPragma);
  ...
  //   in 6.3.2.1.
  if (T->isFunctionType())
    Decayed = getPointerType(T, HonorASPragma);
...
QualType ASTContext::getConstantArrayType(QualType EltTy,
  ...
  if (EltTy.getQualifiers().hasLatentAddressSpace())
    EltTy = removeAddrSpaceQualType(EltTy);
...
QualType ASTContext::getDependentSizedArrayType(QualType elementType,
  ...
  if (elementType.getQualifiers().hasLatentAddressSpace())
    elementType = removeAddrSpaceQualType(elementType);
...
QualType ASTContext::getInjectedClassNameType(CXXRecordDecl *Decl,
  ...
  if (!Decl->QualTypeForDecl.isNull()) {
    assert(isa<InjectedClassNameType>(Decl->QualTypeForDecl));
  } else if (CXXRecordDecl *PrevDecl = Decl->getPreviousDecl()) {
    assert(!PrevDecl->QualTypeForDecl.isNull() && "previous declaration has no type");
    Decl->QualTypeForDecl = PrevDecl->QualTypeForDecl;
    assert(isa<InjectedClassNameType>(Decl->QualTypeForDecl));
  } else {
    Type *newType =
      new (*this, TypeAlignment) InjectedClassNameType(Decl, TST);
    Decl->QualTypeForDecl = QualType(newType, 0);
    Types.push_back(newType);
  }
  return Decl->QualTypeForDecl;
}
...
QualType ASTContext::getTypeDeclTypeSlow(const TypeDecl *Decl) const {
  ...
  assert(Decl->QualTypeForDecl.isNull() && "QualTypeForDecl present in slow case");
...
ASTContext::getTypedefType(const TypedefNameDecl *Decl,
  ...
  if (!Decl->QualTypeForDecl.isNull()) return Decl->QualTypeForDecl;
...
QualType ASTContext::getRecordType(const RecordDecl *Decl) const {
  ...
  if (const RecordDecl *PrevDecl = Decl->getPreviousDecl())
    if (!PrevDecl->QualTypeForDecl.isNull())
      return (Decl->QualTypeForDecl = PrevDecl->QualTypeForDecl);

  auto *newType = new (*this, TypeAlignment) RecordType(Decl);
  Qualifiers qs;
  if (auto *A = Decl->getAttr<DeclLatentAddressSpaceAttr>())
    qs.addLatentAddressSpace(static_cast<LangAS>(A->getAddressSpace()));
  Decl->QualTypeForDecl = getQualifiedType(newType, qs);
  Types.push_back(newType);
  return Decl->QualTypeForDecl;
}
...
QualType ASTContext::getAdjustedParameterType(QualType T) const {
  if (T->isArrayType() || T->isFunctionType())
    return getDecayedType(T, /*HonorASPragma=*/true);
  return T;
}
...
QualType ASTContext::getArrayDecayedType(QualType Ty, bool HonorASPragma) const {
  ...
  QualType PtrTy = getPointerType(PrettyArrayType->getElementType(), HonorASPragma);
...
bool ASTContext::typesAreCompatible(QualType LHS, QualType RHS,
  ...
  return !mergeTypes(LHS, RHS, false, CompareUnqualified, false, ForRedeclaration).isNull();
}
...
QualType ASTContext::mergeFunctionParameterTypes(QualType lhs, QualType rhs,
  ...
  return mergeTypes(lhs, rhs, OfBlockPointer, Unqualified, false, ForRedeclaration);
}
...
QualType ASTContext::mergeFunctionTypes(QualType lhs, QualType rhs,
  ...
  if (!Unqualified)
  {
    // The only extrinsic qualifier a function type can have is a latent address
    // space.  The left's takes precedence if it exists; otherwise keep the
    // right's.
    Qualifiers LQuals = lhs.getQualifiers();
    Qualifiers RQuals = rhs.getQualifiers();
    if (!ForRedeclaration && LQuals != RQuals)
      return {};
    if (LQuals.hasNonLatentQualifiers() || RQuals.hasNonLatentQualifiers())
      return {};
    if (LQuals.hasLatentAddressSpace())
      qs.setLatentAddressSpace(LQuals.getLatentAddressSpace());
    else if (RQuals.hasLatentAddressSpace())
      qs.setLatentAddressSpace(RQuals.getLatentAddressSpace());
  }
  if (allLTypes && qs != lhs.getQualifiers())
    allLTypes = false;
  if (allRTypes && qs != rhs.getQualifiers())
    allRTypes = false;
  ...
  {
    QualType lret = lbase->getReturnType();
    QualType rret = rbase->getReturnType();
    retType = mergeTypes(lret, rret, false,
                         Unqualified, false, ForRedeclaration);
    if (retType.isNull())
    {
      Qualifiers lretQuals = lret.getQualifiers();
      Qualifiers rretQuals = rret.getQualifiers();
      if (lretQuals.hasLatentAddressSpace() != rretQuals.hasLatentAddressSpace() ||
          lretQuals.getLatentAddressSpace() != rretQuals.getLatentAddressSpace())
      {
        if (lretQuals.hasLatentAddressSpace())
          lret = removeAddrSpaceQualType(lret);
        if (rretQuals.hasLatentAddressSpace())
          rret = removeAddrSpaceQualType(rret);
        retType = mergeTypes(lret, rret, false,
                             Unqualified, false, ForRedeclaration);
        if (retType == lret)
          retType = lbase->getReturnType();
      }
    }
  }
  if (retType.isNull())
  ...
  if (lproto && rproto) { // two C99 style function prototypes
    ...
    for (unsigned i = 0, n = lproto->getNumParams(); i < n; i++) {
      ...
      QualType paramType = mergeFunctionParameterTypes(
          lParamType, rParamType, OfBlockPointer, Unqualified, ForRedeclaration);
      if (paramType.isNull())
        return {};
    ...
    return getFunctionType(retType, types, EPI, qs);
  }
  ...
  if (proto) {
    ...
    EPI.ExtInfo = einfo;
    return getFunctionType(retType, proto->getParamTypes(), EPI, qs);
  }
...
QualType ASTContext::mergeTypes(QualType LHS, QualType RHS,
  ...
  if (LQuals != RQuals) {
    // When redeclaring a function, or for block pointer types, allow the
    // left latent address space to override the right latent (or absent)
    // address space.
    if ((ForRedeclaration || OfBlockPointer) &&
        LHSClass == Type::FunctionNoProto &&
        RHSClass == Type::FunctionNoProto &&
        ((LQuals.hasLatentAddressSpace() &&
    ...
    {
      RHS = removeAddrSpaceQualType(RHS);
      if (LQuals.hasLatentAddressSpace())
        RHS = getLatentAddrSpaceQualType(RHS, LQuals.getLatentAddressSpace());
      return mergeTypes(LHS, RHS, OfBlockPointer, Unqualified,
                        BlockReturnType, ForRedeclaration);
    }

    // If any of these qualifiers are different, we have a type
    // mismatch.
    if (LQuals.getCVRQualifiers() != RQuals.getCVRQualifiers() ||
        LQuals.hasAddressSpace() != RQuals.hasAddressSpace() ||
        (LQuals.hasAddressSpace() && LQuals.getAddressSpace() != RQuals.getAddressSpace()) ||
        LQuals.hasLatentAddressSpace() != RQuals.hasLatentAddressSpace() ||
        (LQuals.hasLatentAddressSpace() && LQuals.getLatentAddressSpace() != RQuals.getLatentAddressSpace()) ||
...
#include "clang/AST/TypeNodes.def"
  ...
  case Type::Pointer:
  {
    ...
    QualType RHSPointeeToMerge = RHSPointee;
    if (Unqualified) {
      LHSPointee = LHSPointee.getUnqualifiedType();
      RHSPointeeToMerge = RHSPointee = RHSPointee.getUnqualifiedType();
    } else if (ForRedeclaration) {
      // For redeclarations, override implicit address space on new pointee for that of old pointee
      Qualifiers LHSQuals = LHSPointee.getQualifiers();
      Qualifiers RHSQuals = RHSPointee.getQualifiers();
      if (RHSQuals.hasAddressSpace() != LHSQuals.hasAddressSpace() ||
          RHSQuals.getAddressSpace() != LHSQuals.getAddressSpace())
      {
        // Check if pointer is explicitly declared with __ptr32 or __ptr64
        const AttributedType *AttrT = RHS->getAs<AttributedType>();
        while (AttrT && AttrT->getAttrKind() != attr::Ptr32 && AttrT->getAttrKind() != attr::Ptr64)
          AttrT = AttrT->getModifiedType()->getAs<AttributedType>();
        if (!AttrT)
        {
          // Check if pointee is explicitly declared with __attribute__((address_space(n)))
          AttrT = RHSPointee->getAs<AttributedType>();
          while (AttrT && AttrT->getAttrKind() != attr::AddressSpace)
            AttrT = AttrT->getModifiedType()->getAs<AttributedType>();
        }
        if (!AttrT)
        {
          RHSPointeeToMerge = removeAddrSpaceQualType(RHSPointeeToMerge);
          if (LHSQuals.hasAddressSpace())
            RHSPointeeToMerge = getAddrSpaceQualType(RHSPointeeToMerge, LHSQuals.getAddressSpace());
        }
      }
    }
    QualType ResultType = mergeTypes(LHSPointee, RHSPointeeToMerge, false,
                                     Unqualified, false, ForRedeclaration);
  ...
  case Type::FunctionNoProto:
    return mergeFunctionTypes(LHS, RHS, OfBlockPointer, Unqualified, ForRedeclaration);
  case Type::Record:
  case Type::Enum:
    return {};
  case Type::Builtin:
...
bool ASTContext::mergeExtParameterInfo(
  ...
  bool AnyCovariesReturnType = false;
  size_t E = FirstHasInfo ? FirstFnType->getExtParameterInfos().size()
                          : SecondFnType->getExtParameterInfos().size();

  for (size_t I = 0; I < E; ++I) {
    FunctionProtoType::ExtParameterInfo FirstParam, SecondParam, MergedParam;
    if (FirstHasInfo)
      FirstParam = FirstFnType->getExtParameterInfo(I);
    if (SecondHasInfo)
      SecondParam = SecondFnType->getExtParameterInfo(I);

    // Cannot merge unless everything except the noescape and covaries flags match.
    if (FirstParam.withIsNoEscape(false).withCovariesReturnType(false) !=
        SecondParam.withIsNoEscape(false).withCovariesReturnType(false))
      return false;

    MergedParam = FirstParam;
    bool FirstNoEscape = FirstParam.isNoEscape();
    bool SecondNoEscape = SecondParam.isNoEscape();
    bool IsNoEscape = FirstNoEscape && SecondNoEscape;
    MergedParam = MergedParam.withIsNoEscape(IsNoEscape);

    bool CovariesReturnType =
        FirstParam.covariesReturnType() || SecondParam.covariesReturnType();
    if (CovariesReturnType)
    {
      // Only one parameter can covary the return type.
      if (AnyCovariesReturnType)
        return false;
      AnyCovariesReturnType = true;
    }
    MergedParam = MergedParam.withCovariesReturnType(CovariesReturnType);

    NewParamInfos.push_back(MergedParam);
    if (MergedParam.getOpaqueValue())
      NeedParamInfo = true;
    if (FirstParam != MergedParam)
      CanUseFirst = false;
    if (SecondParam != MergedParam)
      CanUseSecond = false;
  }
    ...
    if (ResReturnType == NewReturnType || ResReturnType == OldReturnType) {
      ...
      if (const auto *FPT = cast<FunctionProtoType>(F)) {
        ...
        QualType ResultType =
            ...
            LHSCan.getLocalQualifiers() + RHSCan.getLocalQualifiers());
        return ResultType;
      }
    }
    return {};
...
static QualType DecodeTypeFromStr(const char *&Str, const ASTContext &Context,
                                  ...
                                  bool *CovariesRetType = nullptr) {
  assert((AllowTypeModifiers || !CovariesRetType) &&
         "can't ask for covaries ret type without allowing modifiers");
...
bool ASTContext::hasSameCVRUnqualType(QualType lhs, QualType rhs) const
{
  if (hasSameType(lhs, rhs))
    return true;

  CanQualType LHSCan = getCanonicalType(lhs);
  CanQualType RHSCan = getCanonicalType(rhs);
  SplitQualType LSplit = LHSCan.split();
  SplitQualType RSplit = RHSCan.split();
  LSplit.Quals.removeCVRQualifiers();
  RSplit.Quals.removeCVRQualifiers();
...
QualType ASTContext::GetBuiltinType(unsigned Id,
  ...
  SmallVector<FunctionProtoType::ExtParameterInfo, 8> ExtParameterInfos;

  bool RequiresICE = false;
  bool CovariesRetType;
  bool anyCovariesRetType = false;
  Error = GE_None;
  QualType ResType = DecodeTypeFromStr(TypeStr, *this, Error,
                                       RequiresICE, true, &CovariesRetType);
  if (Error != GE_None)
    return {};

  assert(!RequiresICE && "Result of intrinsic cannot be required to be an ICE");
  assert(!CovariesRetType && "Return type can't covary itself");
  ...
  while (TypeStr[0] && TypeStr[0] != '.') {
    ...
    FunctionProtoType::ExtParameterInfo ParamInfo;
    if (CovariesRetType)
    {
      assert(!anyCovariesRetType &&
             "Can't have two params which covary return type");
      assert(Ty->isPointerType() && ResType->isPointerType() &&
             "Covarying param and return only supported for pointer types");
      assert(hasSameCVRUnqualType(Ty->getPointeeType(), ResType->getPointeeType()) &&
             "Param which covaries return type must have same type as return");
      ParamInfo = ParamInfo.withCovariesReturnType(true);
      anyCovariesRetType = true;
    }
    ...
    ExtParameterInfos.push_back(ParamInfo);
  }
  ...
  if (getLangOpts().Interop6432)
    qs.setLatentAddressSpace(LangAS::Default);
  return getFunctionType(ResType, ArgTypes, EPI, qs);
}
