...
public:
  ...
  bool shouldSkipDecl(const RecordDecl *RD) const {
    ...
    if (Kind != SrcMgr::C_User && Kind != SrcMgr::C_LocalUser)
      return true;
    ...
    if (auto *CXXRD = dyn_cast<CXXRecordDecl>(RD)) {
      ...
      if (CXXRD->getQualTypeForDecl()->isDependentType())
        return true;
      if (CXXRD->getQualTypeForDecl()->isInstantiationDependentType())
        return true;
    }
  ...
  void reportRecord(
    ...
    Os << RD->getQualTypeForDecl().getAsString(LangOptions())
       << "'";
