...
ParsedType Sema::getTypeName(const IdentifierInfo &II, SourceLocation NameLoc,
  ...
  if (SS && SS->isNotEmpty() && !IsCtorOrDtorName &&
    ...
    if (WantNontrivialTypeSourceInfo) {
      ...
      TypeLocBuilder Builder;
      if (LangOpts.Interop6432 && T->isRecordType())
        T = QualType(T.getTypePtr(), 0);
  ...
  for (;; DC = DC->getLookupParent()) {
    ...
    if (ND && !ND->isInline() && !ND->isAnonymousNamespace())
      ...
      return NestedNameSpecifier::Create(Context, nullptr, RD->isTemplateDecl(),
                                         RD->getQualTypeForDecl().getTypePtr());
...
void Sema::DiagnoseUnusedNestedTypedefs(const RecordDecl *D) {
  if (D->getQualTypeForDecl()->isDependentType())
    return;
...
void Sema::MergeTypedefNameDecl(Scope *S, TypedefNameDecl *New,
    ...
    case 2:
      {
        ...
        New->setQualTypeForDecl(Context.getObjCIdType());
        return;
      }
    case 5:
      ...
      New->setQualTypeForDecl(Context.getObjCClassType());
      return;
    case 3:
      if (!TypeID->isStr("SEL"))
        break;
      ...
      New->setQualTypeForDecl(Context.getObjCSelType());
      return;
    }
  ...
  if (auto *OldTD = dyn_cast<TypedefNameDecl>(Old)) {
    ...
    if (OldTag && NewTag &&
      ...
      // instead of our tag.
      New->setQualTypeForDecl(OldTD->getQualTypeForDecl());
      if (OldTD->isModed())
...
static void mergeParamDeclTypes(ParmVarDecl *NewParam,
  ...
  if (MergeTypeWithOld) {
    QualType NewT = S.Context.mergeTypes(OldParam->getType(), NewParam->getType(), false,
                                         false, false, /*ForRedeclaration=*/true);
    if (!NewT.isNull())
      NewParam->setType(NewT);
  }
}
...
bool Sema::MergeFunctionDecl(FunctionDecl *New, NamedDecl *&OldD,
  ...
  if (RequiresAdjustment) {
    NewQType = Context.adjustFunctionType(New->getType(), NewTypeInfo);
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (!OldTypeInfo.getNoReturn() && NewTypeInfo.getNoReturn()) {
      OldQTypeForComparison
        = Context.adjustFunctionType(OldQType, OldTypeInfo.withNoReturn(true));
  ...
  if (!getLangOpts().CPlusPlus &&
      Context.typesAreCompatible(Old->getType(), New->getType(), false, /*ForRedeclaration=*/true)) {
    const FunctionType *OldFuncType = Old->getType()->getAs<FunctionType>();
    const FunctionType *NewFuncType = New->getType()->getAs<FunctionType>();
...
bool Sema::MergeCompatibleFunctionDecls(FunctionDecl *New, FunctionDecl *Old,
  ...
  if (New->getNumParams() == Old->getNumParams())
      for (unsigned i = 0, e = New->getNumParams(); i != e; ++i) {
        ...
        mergeParamDeclTypes(NewParam, OldParam, *this, MergeTypeWithOld);
      }
  ...
  // was visible.
  QualType Merged = Context.mergeTypes(Old->getType(), New->getType(), false,
                                       false, false, /*ForRedeclaration=*/true);
...
void Sema::MergeVarDeclTypes(VarDecl *New, VarDecl *Old,
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    //   compatible type.
    MergedT = Context.mergeTypes(Old->getType(), New->getType());
  }
  if (MergedT.isNull()) {
...
Sema::ParsedFreeStandingDeclSpec(Scope *S, AccessSpecifier AS, DeclSpec &DS,
  ...
  if (!getLangOpts().CPlusPlus && CurContext->isRecord() &&
    ...
    if ((Tag && Tag->getDeclName()) ||
      ...
      if (Record && (getLangOpts().MicrosoftExt || getLangOpts().Interop6432)) {
...
void Sema::CheckMain(FunctionDecl* FD, const DeclSpec& DS) {
  ...
  if (FT->getCallConv() != CC_C) {
    T = Context.adjustFunctionType(T, FT->getExtInfo().withCallingConv(CC_C));
    FT = T->castAs<FunctionType>();
    FD->setType(T);
  }
  ...
  for (unsigned i = 1; i < nparams; ++i) {
    QualType ParamT = FTP->getParamType(i);
    const PointerType* PointerTy = ParamT->getAs<PointerType>();
    if (!PointerTy) {
      RequiresAdjustment = false;
      break;
    }
    const AttributedType *AttrT = ParamT->getAs<AttributedType>();
    while (AttrT && AttrT->getAttrKind() != attr::Ptr32 && AttrT->getAttrKind() != attr::Ptr64)
      AttrT = AttrT->getModifiedType()->getAs<AttributedType>();
    if (AttrT) {
      // explicit address space attribute; don't modify
      RequiresAdjustment = false;
      break;
    }

    QualType PointeeTy = PointerTy->getPointeeType();
    const PointerType* SecondPointerTy = PointeeTy->getAs<PointerType>();
    if (!SecondPointerTy) {
      RequiresAdjustment = false;
      break;
    }
    AttrT = PointeeTy->getAs<AttributedType>();
    while (AttrT && AttrT->getAttrKind() != attr::Ptr32 && AttrT->getAttrKind() != attr::Ptr64)
      AttrT = AttrT->getModifiedType()->getAs<AttributedType>();
    if (AttrT) {
      // explicit address space attribute; don't modify
    ...
    QualType SecondPointeeTy = SecondPointerTy->getPointeeType();
    if (PointeeTy.getQualifiers().hasAddressSpace() || SecondPointeeTy.getQualifiers().hasAddressSpace())
    {
      if (SecondPointeeTy.getQualifiers().hasAddressSpace())
      {
        SecondPointeeTy = Context.removeAddrSpaceQualType(SecondPointeeTy);

        SplitQualType split = PointeeTy.split();
        split.Quals.removeAddressSpace();

        QualType NewPointeeTy = Context.getPointerType(SecondPointeeTy);
        AttrT = NewPointeeTy->getAs<AttributedType>();
        while (AttrT) {
          NewPointeeTy = Context.getAttributedType(AttrT->getAttrKind(), NewPointeeTy, NewPointeeTy);
          AttrT = AttrT->getModifiedType()->getAs<AttributedType>();
        }

        PointeeTy = Context.getQualifiedType(NewPointeeTy, split.Quals);
      }

      SplitQualType split = ParamT.split();
      split.Quals.removeAddressSpace();

      QualType NewParamT = Context.getPointerType(PointeeTy);
      AttrT = ParamT->getAs<AttributedType>();
      while (AttrT) {
        NewParamT = Context.getAttributedType(AttrT->getAttrKind(), NewParamT, NewParamT);
        AttrT = AttrT->getModifiedType()->getAs<AttributedType>();
      }

      ParamT = Context.getQualifiedType(NewParamT, split.Quals);
      FD->parameters()[i]->setType(ParamT);
  ...
  if (RequiresAdjustment)
  {
    ...
    for (const auto &ParamD : FD->parameters())
      ParamTypes.push_back(ParamD->getType());
    T = Context.getFunctionType(FTP->getReturnType(), ParamTypes, FTP->getExtProtoInfo(),
                                T.getQualifiers());
    FD->setType(T);
    FT = T->castAs<FunctionType>();
    FTP = cast<const FunctionProtoType>(FT);
  }
  ...
  if (FD) {
    ...
    if (getLangOpts().CPlusPlus14) {
      ...
      if (LSI->HasImplicitReturnType) {
        ...
        QualType FDQT = FD->getType();
        const FunctionProtoType *Proto = FDQT->getAs<FunctionProtoType>();
  ...
  auto createTagFromNewDecl = [&]() -> TagDecl * {
    ...
    if (RecordDecl *RD = dyn_cast<RecordDecl>(New)) {
      ...
      if (TUK == TUK_Definition && (!SkipBody || !SkipBody->ShouldSkip))
        AddPragmaAttributesForRecord(RD);
    }
