...
Sema::BuildFieldReferenceExpr(Expr *BaseExpr, bool IsArrow,
  ...
  if (const ReferenceType *Ref = MemberType->getAs<ReferenceType>()) {
    ...
    if (MemberQuals.hasLatentAddressSpace())
    {
        MemberType = Context.removeAddrSpaceQualType(MemberType);
        MemberQuals = Context.getCanonicalType(MemberType).getQualifiers();
    }
