...
void StandardConversionSequence::setAsIdentityConversion() {
  ...
  QualificationIncludesAddressSpace = false;
...
isPointerConversionToVoidPointer(ASTContext& Context) const {
  ...
  if (Second == ICK_Pointer_Conversion && FromType->isAnyPointerType())
    if (const PointerType* ToPtrType = ToType->getAs<PointerType>())
      return Context.removeAddrSpaceQualType(ToPtrType->getPointeeType())->isVoidType();
...
bool
StandardConversionSequence::
isPointerConversionFromVoidPointer(ASTContext& Context) const {
  QualType FromType = getFromType();
  QualType ToType = getToType(1);
  ...
  if (Second == ICK_Pointer_Conversion && ToType->isAnyPointerType())
    if (const PointerType* FromPtrType = FromType->getAs<PointerType>())
      return Context.removeAddrSpaceQualType(FromPtrType->getPointeeType())->isVoidType();
...
bool Sema::IsFunctionConversion(QualType FromType, QualType ToType,
  ...
  if (FromEInfo.getNoReturn() && !ToEInfo.getNoReturn()) {
    CanFrom = Context.getCanonicalType(
        Context.adjustFunctionType(CanFrom, FromEInfo.withNoReturn(false)));
    FromFn = cast<FunctionType>(CanFrom);
    Changed = true;
  }
  ...
  if (const auto *FromFPT = dyn_cast<FunctionProtoType>(FromFn)) {
    ...
    if (FromFPT->isNothrow() && !ToFPT->isNothrow()) {
      CanFrom = Context.getCanonicalType(Context.getFunctionTypeWithExceptionSpec(CanFrom, EST_None));
      FromFn = cast<FunctionType>(CanFrom);
      Changed = true;
    }
    ...
    if (Context.mergeExtParameterInfo(ToFPT, FromFPT, CanUseToFPT,
      ...
      QualType QT = Context.getFunctionType(FromFPT->getReturnType(),
                                            ...
                                            CanFrom.getQualifiers());
      CanFrom = Context.getCanonicalType(QT);
      FromFn = cast<FunctionType>(CanFrom);
      Changed = true;
    }
  }
...
static bool IsStandardConversion(Sema &S, Expr* From, QualType ToType,
  ...
  if (argIsLValue &&
    ...
    if (S.IsStringLiteralToNonConstPointerConversion(From, ToType)) {
      ...
      SCS.QualificationIncludesAddressSpace = false;
  ...
  if (S.IsFunctionConversion(FromType, ToType, FromType)) {
    ...
    SCS.Third = ICK_Qualification;
    SCS.QualificationIncludesAddressSpace = AddressSpaceConversion;
...
BuildSimilarlyQualifiedPointerType(const Type *FromPtr,
  ...
  return Context.getPointerType(QualifiedCanonToPointee, /*HonorASPragma=*/false);
}
...
bool Sema::IsPointerConversion(Expr *From, QualType FromType, QualType ToType,
  ...
  // Allow conversion from an rvalue of type "pointer to cv/AS void" to
  // an rvalue of type "pointer to cv T", where T is an object type.
  if (InOverloadResolution && ToPointeeType->isIncompleteOrObjectType() &&
      Context.removeAddrSpaceQualType(FromPointeeType)->isVoidType()) {
    ConvertedType = BuildSimilarlyQualifiedPointerType(FromTypePtr,
                                                       ToPointeeType,
                                                       ToType, Context,
                                                   /*StripObjCLifetime=*/true);
...
Sema::IsQualificationConversion(QualType FromType, QualType ToType,
  ...
  while (Context.UnwrapSimilarTypes(FromType, ToType)) {
    ...
    // Check address space conversions.
    if ((FromQuals.hasAddressSpace() != ToQuals.hasAddressSpace() ||
         FromQuals.getAddressSpace() != ToQuals.getAddressSpace()) &&
        UnwrappedAnyPointer) {
      if (ToQuals.isAddressSpaceSupersetOf(FromQuals)) {
        AddressSpaceConversion = true;
        FromQuals.removeAddressSpace();
        ToQuals.removeAddressSpace();
      } else {
        // Qualification conversions cannot cast between incompatible
...
static bool tryAtomicConversion(Sema &S, Expr *From, QualType ToType,
  ...
  SCS.QualificationIncludesAddressSpace
    = InnerSCS.QualificationIncludesAddressSpace;
                                   ...
                                   const StandardConversionSequence& SCS2)
{
  ...
  bool SCS1ConvertsFromVoid
    = SCS1.isPointerConversionFromVoidPointer(S.Context);
  bool SCS2ConvertsFromVoid
    = SCS2.isPointerConversionFromVoidPointer(S.Context);
  if (SCS1ConvertsFromVoid != SCS2ConvertsFromVoid) {
    // Exactly one of the conversion sequences is a conversion to
    // a void pointer; it's the worse conversion.
    return SCS2ConvertsFromVoid ? ImplicitConversionSequence::Better
                                : ImplicitConversionSequence::Worse;
  } else if (!SCS1ConvertsFromVoid && !SCS2ConvertsFromVoid) {
    // Neither conversion sequence converts to a void pointer; compare
    // their derived-to-base conversions.
    if (ImplicitConversionSequence::CompareKind DerivedCK
          = CompareDerivedToBaseConversions(S, Loc, SCS1, SCS2))
      return DerivedCK;
             ...
             !S.Context.hasSameType(SCS1.getToType(1), SCS2.getToType(1))) {
    // Both conversion sequences are conversions from void
    // pointers. Compare the destination types to determine if there's an
    // inheritance relationship in their sources.
    QualType ToType1 = SCS1.getToType(1);
    QualType ToType2 = SCS2.getToType(1);

    QualType ToPointee1 = ToType1->getPointeeType().getUnqualifiedType();
    QualType ToPointee2 = ToType2->getPointeeType().getUnqualifiedType();

    if (S.IsDerivedFrom(Loc, ToPointee2, ToPointee1))
      return ImplicitConversionSequence::Better;
    else if (S.IsDerivedFrom(Loc, ToPointee1, ToPointee2))
      return ImplicitConversionSequence::Worse;

    // Objective-C++: If one interface is more specific than the
    // other, it is the better one.
    const ObjCObjectPointerType* ToObjCPtr1
      = ToType1->getAs<ObjCObjectPointerType>();
    const ObjCObjectPointerType* ToObjCPtr2
      = ToType2->getAs<ObjCObjectPointerType>();
    if (ToObjCPtr1 && ToObjCPtr2) {
      bool AssignLeft = S.Context.canAssignObjCInterfaces(ToObjCPtr1,
                                                          ToObjCPtr2);
      bool AssignRight = S.Context.canAssignObjCInterfaces(ToObjCPtr2,
                                                           ToObjCPtr1);
      if (AssignLeft != AssignRight) {
        return AssignLeft? ImplicitConversionSequence::Better
                         : ImplicitConversionSequence::Worse;
...
CompareQualificationConversions(Sema &S,
  ...
  // Address spaces:
  //   Prefer qualification conversions not involving a change in address space
  //   to qualification conversions that do not change address space.
  if (SCS1.QualificationIncludesAddressSpace !=
                                      SCS2.QualificationIncludesAddressSpace) {
    Result = SCS1.QualificationIncludesAddressSpace
               ? ImplicitConversionSequence::Worse
               : ImplicitConversionSequence::Better;
  }
...
static bool isAllowableExplicitConversion(Sema &S,
  ...
  bool ObjCLifetimeConversion, AddressSpaceConversion;
...
static void DiagnoseBadDeduction(Sema &S, NamedDecl *Found, Decl *Templated,
  ...
  case Sema::TDK_Underqualified: {
    ...
    Qs.strip(Param);
    QualType NonCanonParam = Qs.apply(S.Context, TParam->getQualTypeForDecl());
