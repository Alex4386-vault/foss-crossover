...
void Sema::AddPragmaAttributesForRecord(RecordDecl *RD) {
  ...
  {
    RD->addAttr(DeclLatentAddressSpaceAttr::CreateImplicit(
        Context, static_cast<unsigned>(AddrSpaceStack.CurrentValue),
        AddrSpaceStack.CurrentPragmaLocation));
    QualType T = RD->getQualTypeForDecl();
    if (!T.isNull())
    {
      QualType NewT = Context.getLatentAddrSpaceQualType(T, AddrSpaceStack.CurrentValue);
      if (NewT != T)
      {
        for (auto *DI = RD; DI; DI = DI->getPreviousDecl())
          DI->setQualTypeForDecl(NewT);
      }
    }
  }
}
