  ...
  struct CastOperation {
    CastOperation(Sema &S, QualType destType, ExprResult src)
      : Self(S), SrcExpr(src), DestType(destType),
        ...
        IsTruncateCast(false), IsASCast(false) {
...
static void checkIntToPointerCast(bool CStyle, SourceLocation Loc,
                                  const Expr *SrcExpr, QualType SrcType,
                                  QualType DestType, Sema &Self,
                                  bool isTruncateCast) {
  if (SrcType.isNull())
  ...
  if (Self.Context.getTypeSize(DestType) > Self.Context.getTypeSize(SrcType)) {
  ...
  if (!isTruncateCast &&
    ...
    Self.Diag(Loc, diag::warn_int_to_smaller_pointer_cast) << SrcType << DestType;
  }
}

static void checkPointerToIntCast(bool CStyle, SourceLocation Loc,
                                  const Expr *SrcExpr, QualType SrcType,
                                  QualType DestType, Sema &Self,
                                  bool isTruncateCast, bool isPointee = false) {
  if (SrcType.isNull())
    SrcType = SrcExpr->getType();

  // Not warning on reinterpret_cast, boolean, etc
  // are not explicit design choices, but consistent with GCC's behavior.
  // Feel free to modify them if you've reason/evidence for an alternative.
  if (!CStyle || !DestType->isIntegralType(Self.Context)
  ...
  if (isPointee &&
      Self.Context.getTypeSize(DestType) > Self.Context.getTypeSize(SrcType)) {
    Self.Diag(Loc, diag::warn_pointer_to_int_cast) << SrcType << DestType;
  }

  if (!isTruncateCast &&
      Self.Context.getTypeSize(DestType) < Self.Context.getTypeSize(SrcType)) {
    Self.Diag(Loc, diag::warn_pointer_to_smaller_int_cast) << SrcType << DestType;
  }
}
...
static TryCastResult TryReinterpretCast(Sema &Self, ExprResult &SrcExpr,
  ...
  if (SrcType->isIntegralOrEnumerationType()) {
    ...
    checkIntToPointerCast(CStyle, OpRange.getBegin(), SrcExpr.get(), SrcType,
                          DestType, Self, /*isTruncateCast=*/false);
...
void CastOperation::CheckCStyleCast() {
  ...
  if (DestType->isAnyPointerType() && SrcType->isAnyPointerType()) {
    Qualifiers SrcQuals = SrcType->getPointeeType().getQualifiers();
    Qualifiers DestQuals = DestType->getPointeeType().getQualifiers();
    bool CompatibleAddressSpace;
    if (Self.getLangOpts().Interop6432) {
      CompatibleAddressSpace = DestQuals.isAddressSpaceSupersetOf(SrcQuals) ||
          ((IsASCast || IsTruncateCast) &&
           SrcQuals.isAddressSpaceSupersetOf(DestQuals));
      if (!IsTruncateCast) {
        // For a pointer to a pointer, the address spaces of the inner pointers
        // must match. We really don't want to cast, e.g. a 64-bit pointer to a
        // 64-bit pointer to a 32-bit pointer to a 32-bit pointer, or vice
        // versa. Allow explicitly truncating casts, though.
        QualType SrcPointeeType = SrcType->getPointeeType();
        QualType DestPointeeType = DestType->getPointeeType();
        while (SrcPointeeType->isAnyPointerType() &&
               DestPointeeType->isAnyPointerType()) {
          SrcQuals = SrcPointeeType->getPointeeType().getQualifiers();
          DestQuals = DestPointeeType->getPointeeType().getQualifiers();
          CompatibleAddressSpace &=
              (SrcQuals.isAddressSpaceSupersetOf(DestQuals) &&
               DestQuals.isAddressSpaceSupersetOf(SrcQuals));
          SrcPointeeType = SrcPointeeType->getPointeeType();
          DestPointeeType = DestPointeeType->getPointeeType();
        }
        // Also check for pointer-to-pointer to pointer-to-integer casts, and
        // vice versa. Ignore 1- and 2-byte integer types, though; char* casts
        // are common enough that we don't want to warn on those.
        if (SrcPointeeType->isAnyPointerType() &&
            DestPointeeType->isIntegerType()) {
          unsigned TypeSize = Self.Context.getTypeSize(DestPointeeType);
          if (TypeSize == 32 || TypeSize == 64) {
            checkPointerToIntCast(/* CStyle */ true, OpRange.getBegin(),
                                  SrcExpr.get(), SrcPointeeType,
                                  DestPointeeType, Self, IsTruncateCast,
                                  /*isPointee=*/true);
          }
        } else if (SrcPointeeType->isIntegerType() &&
                   DestPointeeType->isAnyPointerType()) {
          unsigned TypeSize = Self.Context.getTypeSize(SrcPointeeType);
          if (TypeSize == 32 || TypeSize == 64) {
            checkIntToPointerCast(/* CStyle */ true, OpRange.getBegin(),
                                  SrcExpr.get(), SrcPointeeType,
                                  DestPointeeType, Self, IsTruncateCast);
          }
        }
    ...
    if (DestQuals.getAddressSpace() != SrcQuals.getAddressSpace())
    {
      QualType SrcPtee = SrcType->getPointeeType();
      SrcPtee = Self.Context.removeAddrSpaceQualType(SrcPtee);
      SrcPtee = Self.Context.getAddrSpaceQualType(SrcPtee, DestQuals.getAddressSpace());
      SrcType = Self.Context.getQualifiedType(Self.Context.getPointerType(SrcPtee), SrcType.getQualifiers());
      SrcExpr = Self.ImpCastExprToType(SrcExpr.get(), SrcType, CK_AddressSpaceConversion);
      if (SrcExpr.isInvalid())
        return;
    }
  }
  ...
  if (!DestType->isArithmeticType()) {
    ...
    checkIntToPointerCast(/* CStyle */ true, OpRange.getBegin(), SrcExpr.get(),
                          SrcType, DestType, Self, IsTruncateCast);
    ...
    checkPointerToIntCast(/* CStyle */ true, OpRange.getBegin(), SrcExpr.get(),
                          SrcType, DestType, Self, IsTruncateCast);
  }
...
ExprResult Sema::BuildCStyleCastExpr(SourceLocation LPLoc,
                                     ...
                                     Expr *CastExpr, bool isTruncateCast,
                                     bool isASCast) {
  CastOperation Op(*this, CastTypeInfo->getType(), CastExpr);
  Op.DestRange = CastTypeInfo->getTypeLoc().getSourceRange();
  Op.OpRange = SourceRange(LPLoc, CastExpr->getEndLoc());
  Op.IsTruncateCast = isTruncateCast;
  Op.IsASCast = isASCast;
