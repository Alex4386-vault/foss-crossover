...
void Sema::checkCall(NamedDecl *FDecl, const FunctionProtoType *Proto,
  ...
  if (FDecl) {
    ...
    if (LangOpts.Interop6432 && CallType != VariadicDoesNotApply)
    {
      bool NeedsPtr32 = Proto ? is32BitInteropCC(Proto->getCallConv()) :
                        FDecl->hasAttr<CDecl32Attr>();
      AS = NeedsPtr32 ? LangAS::ptr32 : LangAS::Default;
    }
...
bool Sema::CheckFormatArguments(const FormatAttr *Format,
  ...
  if (getFormatStringInfo(Format, IsCXXMember, &FSI))
    return CheckFormatArguments(Args, FSI.HasVAListArg, FSI.FormatIdx,
                                ...
                                CallType, Loc, Range, CheckedVarArgs, AS);
  return false;
}
...
bool Sema::CheckFormatArguments(ArrayRef<const Expr *> Args,
  ...
  if (AS != static_cast<LangAS>(-1) && Type == FST_Scanf &&
      CallType != VariadicDoesNotApply)
  {
    bool AnyErrors = false;
    for (unsigned I = firstDataArg; I < Args.size(); ++I)
    {
      const Expr *Arg = Args[I];
      if (!Arg)
        continue;

      QualType OuterTy = Arg->getType();
      const PointerType *OuterPtrTy = OuterTy->getAs<PointerType>();
      // For scanf, args should be pointers.  If they're not, not our problem.
      if (!OuterPtrTy)
        continue;

      QualType InnerTy = OuterPtrTy->getPointeeType();
      const PointerType *InnerPtrTy = InnerTy->getAs<PointerType>();
      // We're only interested in pointers to pointers.
      if (!InnerPtrTy)
        continue;

      QualType InnerPointeeTy = InnerPtrTy->getPointeeType();
      Qualifiers Quals = InnerPointeeTy.getQualifiers();
      if (Quals.getAddressSpace() != AS)
      {
        InnerPointeeTy = Context.removeAddrSpaceQualType(InnerPointeeTy);
        InnerPointeeTy = Context.getAddrSpaceQualType(InnerPointeeTy, AS);
        InnerTy = Context.getQualifiedType(Context.getPointerType(InnerPointeeTy), InnerTy.getQualifiers());
        OuterTy = Context.getQualifiedType(Context.getPointerType(InnerTy), OuterTy.getQualifiers());

        Diag(Arg->getBeginLoc(), diag::err_typecheck_incompatible_address_space)
            << Arg->getType() << OuterTy << AA_Passing << Arg->getSourceRange();
        AnyErrors = true;
      }
    }
    if (AnyErrors)
...
void Sema::RefersToMemberWithReducedAlignment(
  ...
  CharUnits CompleteObjectAlignment = Context.getTypeAlignInChars(
      ReverseMemberChain.back()->getParent()->getQualTypeForDecl());
    ...
    for (FieldDecl *FDI : ReverseMemberChain) {
      if (FDI->hasAttr<PackedAttr>() ||
        ...
        Alignment = std::min(
            ...
            Context.getTypeAlignInChars(FD->getParent()->getQualTypeForDecl()));
        break;
      }
    }
