  ...
  if (Target.isOSBinFormatMachO()) {
    ...
    if (!DarwinArchName.empty()) {
      ...
      if (Args.hasArg(options::OPT_mwine32) &&
        ...
        bool stack64 = false;
        if (Arg *A = Args.getLastArg(options::OPT_mstack64, options::OPT_mno_stack64))
          stack64 = A->getOption().matches(options::OPT_mstack64);
        Target.setEnvironment(stack64
                ? llvm::Triple::Wine32SP64
                : llvm::Triple::Wine32);
      }
      return Target;
    }
