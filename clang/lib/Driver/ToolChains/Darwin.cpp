...
void Darwin::AddDeploymentTarget(DerivedArgList &Args) const {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_isysroot)) {
    ...
    if (char *env = ::getenv("SDKROOT")) {
      SDKRoot = env;
    } else if (auto XCRun = llvm::sys::findProgramByName("xcrun")) {
      SmallString<32> OutputFile;
      llvm::sys::fs::createTemporaryFile("xcrun-show-sdk-path-output", "", OutputFile);
      llvm::FileRemover OutputRemover(OutputFile.c_str());

      Optional<StringRef> Redirects[] = {{""}, StringRef(OutputFile), {""}};
      StringRef XCRunArgs[] = {"xcrun", "--show-sdk-path"};
      int RunResult =
          llvm::sys::ExecuteAndWait(*XCRun, XCRunArgs, llvm::None, Redirects);
      if (RunResult == 0) {
    ...
    if (!SDKRoot.empty() && SDKRoot != StringRef("/") &&
        llvm::sys::path::is_absolute(SDKRoot) && getVFS().exists(SDKRoot)) {
      Args.append(Args.MakeSeparateArg(
          nullptr, Opts.getOption(options::OPT_isysroot), SDKRoot));
    }
  }
