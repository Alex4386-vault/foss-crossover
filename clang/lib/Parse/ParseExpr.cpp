...
///         '(' type-name ')' cast-expression
///                        [Clang extension]
///         '(' '__truncate' type-name ')' cast-expression
...
Parser::ParseParenExpression(ParenParseOption &ExprType, bool stopIfCastExpr,
  ...
  // Check for __truncate keyword.
  SourceLocation TruncateKeywordLoc;
  bool isTruncateCast = TryConsumeToken(tok::kw___truncate, TruncateKeywordLoc);

  // Check for the __addrspace keyword.
  SourceLocation ASCastKeywordLoc;
  bool isASCast = TryConsumeToken(tok::kw___addrspace, ASCastKeywordLoc);

  // None of these cases should fall through with an invalid Result
  // unless they've already reported an error.
  if (!isTruncateCast && !isASCast && ExprType >= CompoundStmt &&
      Tok.is(tok::l_brace)) {
  ...
  } else if (!isTruncateCast && !isASCast && ExprType >= CompoundLiteral &&
             BridgeCast) {
    ...
    if (!isTruncateCast && !isASCast && isAmbiguousTypeId && !stopIfCastExpr) {
    ...
    if (!isTruncateCast && !isASCast && !DeclaratorInfo.isInvalidType() &&
        Tok.is(tok::identifier) &&
      ...
      ColonProtection.restore();
      RParenLoc = T.getCloseLocation();
      if (!isTruncateCast && !isASCast && Tok.is(tok::l_brace)) {
      ...
      if (!isTruncateCast && !isASCast && Tok.is(tok::l_paren)) {
  ...
  } else if (!isTruncateCast && !isASCast && ExprType >= FoldExpr &&
             Tok.is(tok::ellipsis) && isFoldOperator(NextToken().getKind())) {
    ExprType = FoldExpr;
    ...
    if (!isTruncateCast && !isASCast &&
        ExprType >= FoldExpr && isFoldOperator(Tok.getKind()) &&
