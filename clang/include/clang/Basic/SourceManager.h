...
namespace SrcMgr {
  ...
  enum CharacteristicKind {
    C_LocalUser, C_User, C_System, C_ExternCSystem, C_User_ModuleMap,
    C_System_ModuleMap
  };

  /// Determine whether a file / directory characteristic is for system code.
  inline bool isSystem(CharacteristicKind CK) {
    return CK != C_User && CK != C_LocalUser && CK != C_User_ModuleMap;
  }
  ...
  public:
    ...
    /// True if this content cache is from a file that was included with
    /// angled brackets.
    unsigned IsAngled : 1;
    ...
    ContentCache(const ContentCache &RHS)
      : Buffer(nullptr, false), BufferOverridden(false), IsSystemFile(false),
        IsTransient(false), IsAngled(false) {
...
public:
  ...
  FileID createFileID(const FileEntry *SourceFile, SourceLocation IncludePos,
    ...
    const SrcMgr::ContentCache *IR =
        getOrCreateContentCache(SourceFile, isSystem(FileCharacter), IsAngled);
  ...
  /// Returns whether \p FID is a file that was included with angled brackets.
  bool isAngledFile(FileID FID) const {
    bool MyInvalid = false;
    const SrcMgr::SLocEntry &Entry = getSLocEntry(FID, &MyInvalid);
    if (MyInvalid || !Entry.isFile()) {
      return false;
    ...
    return Entry.getFile().getContentCache()->IsAngled;
  }

  /// Returns whether \p Loc is in a file that was included with angled
  /// brackets.
  bool isInAngledFile(SourceLocation Loc) const {
    return isAngledFile(getFileID(Loc));
  }
