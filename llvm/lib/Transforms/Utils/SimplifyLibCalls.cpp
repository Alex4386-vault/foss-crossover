...
Value *LibCallSimplifier::optimizeStrChr(CallInst *CI, IRBuilder<> &B) {
  ...
  if (!CharC) {
    ...
    Value *MemChr = emitMemChr(SrcStr, CI->getArgOperand(1), // include nul.
                      ConstantInt::get(DL.getIntPtrType(CI->getContext()), Len),
                      B, DL, TLI);
    return MemChr ? B.CreatePointerBitCastOrAddrSpaceCast(MemChr, CI->getType()) : nullptr;
  }
...
Value *LibCallSimplifier::optimizeStrStr(CallInst *CI, IRBuilder<> &B) {
  ...
  if (HasStr2 && ToFindStr.size() == 1) {
    ...
    return StrChr ? B.CreatePointerBitCastOrAddrSpaceCast(StrChr, CI->getType()) : nullptr;
  }
  return nullptr;
}
