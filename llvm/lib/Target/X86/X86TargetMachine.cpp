...
static std::string computeDataLayout(const Triple &TT) {
  ...
  if (TT.isArch64Bit() && TT.getEnvironment() == Triple::Wine32)
    Ret += "-p32:32:32-A32";
  // This variant has 32-bit pointers, but uses 64-bit stack pointers. This
  // must be used for code that executes at startup, before a 32-bit stack
  // is guaranteed to have been set up.
  else if (TT.isArch64Bit() && TT.getEnvironment() == Triple::Wine32SP64)
    Ret += "-p32:32:32";
...
bool X86PassConfig::addPreISel() {
  ...
  if (TT.isArch64Bit() && TT.is32On64())
    addPass(createX866432ThunkReferencePass());
  return true;
}
